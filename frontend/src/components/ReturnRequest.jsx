...
const ReturnRequest = () => {
  ...
  const fetchBorrows = async () => {
    ...
    try {
      ...
      // Add a unique identifier for each return request
      const filteredBorrows = res.data.data
        .filter(b => b.return_status === 'pending')
        .map((b, index) => ({
          ...b,
          uniqueKey: `${b.id}-${b.user_id}-${b.book_id}-${index}` // More unique key
        }));
              ...
              <tr key={borrow.uniqueKey}>
                ...
                <td>
                  {user?.role === 'admin' && borrow.return_status === 'pending' && (
                    ...
                    >
                      {processing === borrow.id ? 'Memproses...' : 'Konfirmasi Pengembalian'}
                    </button>
                  )}
                </td>
              </tr>
